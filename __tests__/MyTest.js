const { countBall, countStrike } = require('../src/Referee');
...
describe('Referee Test', () => {
  test('다른 자리에 있는 숫자의 수를 세야한다.', () => {
    ...
    expect(countBall(answer, input)).toEqual(2);
  });
  ...
  test('겹치는 수가 없는 경우 0을 반환한다.', () => {
    ...
    expect(countBall(answer, input)).toEqual(0);
  });
  ...
  test('같은 수가 같은 자리에 있으면 스트라이크', () => {
    ...
    expect(countStrike(answer, input)).toEqual(1);
  });
});
