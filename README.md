...
☐ 3. 사용자의 입력값이 올바른지 확인하는 기능 구현
  ☐ 3-1. 중복된 값을 넣었는지 확인
  ☐ 3-2. 3자리 수 이하이거나 이상인 값을 넣었는지 확인

☐ 4. 결과를 출력하는 기능 구현
  - 3스트라이크가 아닌 경우
  ☐ 4-1. 자리와 숫자가 모두 맞으면 strick
  ☐ 4-2. 숫자만 맞으면 ball
  ☐ 4-3. 모두 맞지 않으면 낫싱
  ☐ 4-4. strick,ball,낫싱의 개수를 출력한다.
  - 3스트라이크일 경우
  ☐ 4-5. 게임을 종료시킨 후 재시작할지 종료할지 확인
  ☐ 4-6. 재시작일 경우 처음(1번)으로 돌아간다.
  ☐ 4-7. 종료일경우 게임을 종료시킨다.
...
🤔 프로그래밍 요구 사항 중 App.js의 play()가 프로그램 실행의 시작점이라 명시되어 있고 App.js엔 class App이 명시되어 있다. class를 한번도 다뤄본 적이 없기 때문에 문제를 이해하기 어렵다.
  ...
  👉 class와 기본 문법에 대해서 deep-js 책을 통해 공부하고 정리하자.
